...
const CONVERSATION_ITEM_ID_PREFIX: &str = "item_";
...
impl ConversationItemId {
    pub fn new() -> Self {
        Self(format!("{CONVERSATION_ITEM_ID_PREFIX}{}", Uuid::now_v7()))
    }

    pub fn parse(value: impl Into<String>) -> Result<Self, String> {
        let value = value.into();
        let suffix = value
            .strip_prefix(CONVERSATION_ITEM_ID_PREFIX)
            .ok_or_else(|| {
                format!("conversation item id must start with {CONVERSATION_ITEM_ID_PREFIX}")
            })?;
        let uuid = Uuid::parse_str(suffix)
            .map_err(|error| format!("invalid conversation item id: {error}"))?;
        if uuid.get_version_num() != 7 {
            return Err("conversation item id must contain a UUIDv7 value".to_string());
        }
        Ok(Self(value))
    }
...
impl Default for ConversationItemId {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Borrow<str> for ConversationItemId {
    fn borrow(&self) -> &str {
        self.as_str()
    }
}

impl AsRef<str> for ConversationItemId {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}

impl Deref for ConversationItemId {
    type Target = str;
...
mod tests {
    ...
    #[test]
    fn conversation_item_ids_are_typed_unique_uuidv7_values() {
        let first = ConversationItemId::new();
        let second = ConversationItemId::new();

        assert_ne!(first, second);
        assert!(first.as_str().starts_with(CONVERSATION_ITEM_ID_PREFIX));
        assert_eq!(
            ConversationItemId::parse(first.to_string()).expect("parse fresh item id"),
            first
        );
        assert!(ConversationItemId::parse("item-1").is_err());
        assert!(ConversationItemId::parse(format!("item_{}", Uuid::new_v4())).is_err());
    }
}
