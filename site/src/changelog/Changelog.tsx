...
const copy = {
  en: {
    ...
    summaries: {
      "v0.3.6":
        "Strengthens runtime reliability across delegated work and durable task state. Child agents now inherit one serialized execution-policy snapshot, thread transcripts use a stable cross-process lock during append and rewrite, provider compaction retries and truncated tool output remain visible in task summaries, and session completion publishes the complete task registry before the terminal event.",
      "v0.3.5":
  ...
  zh: {
    ...
    summaries: {
      "v0.3.6":
        "增强 runtime reliability，统一 delegated work 与 durable task 状态。子 agent 现在继承同一份序列化执行策略快照；thread transcript 在 append 和 rewrite 时使用稳定的跨进程锁；provider 压缩重试与被截断的 tool 输出会保留在任务摘要中；session 完成事件前会先发布完整 task registry，避免后台任务状态被终态遮蔽。",
      "v0.3.5":
