...
4. **Permission profile persistence:** Codex preserves approval mode across
   ...
   silently treated as literal paths, and `:minimal` is explicitly rejected
   until Orca has Codex-style platform default read roots. Fully restricted
   Codex read allow-list enforcement, `:minimal` materialization, glob
   expansion/enforcement, and real network proxy/domain policy compilation
   remain later expansions.
