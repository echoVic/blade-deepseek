...
3. Move more turn-loop orchestration behind runtime-owned APIs. Seeded after v0.1.42.
   ...
   - Runtime should own command handling and event emission. The current
     ...
     `RuntimeTurnConfig`, the first package-3 `QueryConfig`-style split before
     separating turn dependencies and mutable execution state.
     context. Workflow IPC execution now flows through a runtime IPC trait on
