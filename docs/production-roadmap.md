...
4. **Permission profile persistence:** Codex preserves approval mode across
   ...
   materializes to platform default read roots needed by shell runtimes. Glob
   filesystem entries are now rejected with an explicit `command/exec` error
   instead of being silently treated as literal paths. Fully restricted Codex
   read allow-list enforcement, glob expansion/enforcement, and real network
