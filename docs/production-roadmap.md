...
4. **Permission profile persistence:** Codex preserves approval mode across
   ...
   `/**` filesystem entries now normalize to subtree roots, while other glob
   filesystem entries are rejected with an explicit `command/exec` error instead
   of being silently treated as literal paths. Fully restricted Codex read
   allow-list enforcement, broader glob expansion/enforcement, and real network
