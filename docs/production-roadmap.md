...
4. Prepare for long-running shell sessions, worktree automation, and async
   ...
   `server/permission_manager.rs` in v0.1.128, and server shell-session state
   moved into `server/shell_manager.rs` in v0.1.129. The feature work remains
   open after v0.1.129 for deeper async-subagent and reducer-style runtime
   convergence.
