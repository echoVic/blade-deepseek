...
5. **Protocol item stream:** Codex SDK emits `thread.started`, `turn.started`,
   ...
   share MCP tool name parsing, JSON argument parsing, MCP result shaping,
   camelCase tool error object helpers, exit-code normalization from runtime
   payloads or persisted result metadata, and completed-status checks. Realtime
