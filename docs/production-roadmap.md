...
4. **Permission profile persistence:** Codex preserves approval mode across
   ...
   Over-broad globs without a static parent directory are still rejected before
   scanning. Configurable scan depth and real network proxy/domain policy
   enforcement remain later expansions.
