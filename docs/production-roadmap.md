...
| P1 | Storage-neutral ThreadStore | Codex keeps thread persistence behind a dedicated `thread-store` crate; Orca now exposes a `thread_store` module for `ThreadStore`, `SessionStore`, live threads, projections, pagination, and filters while the JSONL implementation remains in `history` for compatibility; next move implementation-owned JSONL helpers behind the module boundary without changing server/session behavior | Medium |
