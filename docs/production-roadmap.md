...
Current baseline: v0.1.149 isolates realtime server item projection in a shared `RuntimeEventProjector` reducer so `ServerRequestWriter` no longer owns assistant message, plan, reasoning, tool, file-change, and workflow runtime-event lifecycle state while the app-server item stream, legacy protocol events, server runtime contracts, and thread-store projections keep the same wire behavior, plus v0.1.148 groups async subagent worker inputs behind `AsyncSubagentWorkerInput`, `AsyncSubagentWorkerContext`, `AsyncSubagentLaunchContext`, and `AsyncSubagentWorkerSpawnContext` so worker entrypoint, injectable worker execution, parent launch, and process spawn boundaries no longer expose long argument lists or clippy allowances while CLI worker startup, task-registry state, worktree handoff, async completion payloads, subagent contracts, and TUI async subagent behavior keep the same runtime shape, plus v0.1.147 groups child-agent runtime constructor inputs behind `ChildAgentRuntimeContext` so `ChildAgentRuntime::new` no longer exposes a long cwd/events/sink/instructions/memory/MCP/hooks/cancel/lifecycle/executor argument list while `agent_child.rs` preserves re-exports and child-agent loop setup, provider turns, compaction, response folding, tool execution, sync and async subagent contracts, workflow child agents, and TUI child-agent delegation keep the same runtime shape, plus v0.1.146 groups child-agent loop runner inputs behind `ChildAgentLoopContext` so `run_child_agent_loop_with_tool_executor` no longer exposes a long request/cwd/instructions/memory/hooks/child-cost-tracker argument list while `agent_child.rs` preserves re-exports and loop setup, provider turns, compaction, response folding, tool execution, subagent contracts, and TUI child-agent delegation keep the same runtime shape, plus v0.1.145 groups child-agent prompt entrypoint inputs behind `ChildAgentPromptContext` so `run_child_agent_prompt_with_tool_executor` no longer exposes a long prompt/subagent/cwd/instructions/memory/hooks argument list while `agent_child.rs` preserves re-exports and prompt-to-request construction, model override setup, child cost tracking, subagent contracts, workflow child agents, and TUI child-agent delegation keep the same runtime shape, plus v0.1.144 isolates child-agent public entrypoint wrappers in a focused `child_agent_entrypoints.rs` module so `agent_child.rs` remains a compatibility facade and model override setup, child cost tracking, executor error folding, prompt-to-request construction, subagent contracts, workflow child agents, and TUI child-agent delegation keep the same runtime shape, plus v0.1.143 isolates child-agent behavior tests in a focused `child_agent_tests.rs` module so `agent_child.rs` remains a small compatibility facade and child-agent setup, provider turns, response folding, loop running, subagent contracts, and TUI child-agent delegation keep the same runtime shape, plus v0.1.142 isolates child-agent shared request/result/runtime/executor types in a focused `child_agent_types.rs` module while `agent_child.rs` preserves public re-exports and child-agent loop setup, provider turns, response folding, loop running, subagent contracts, and TUI child-agent delegation keep the same runtime shape, plus v0.1.141 isolates child-agent loop orchestration in a focused `child_agent_loop_runner.rs` module while `agent_child.rs` preserves public re-exports and loop setup, provider turns, tool-result folding, subagent contracts, and TUI child-agent delegation keep the same runtime shape, plus v0.1.140 isolates child-agent provider response folding, tool request extraction, tool execution context, nested child-cost merging, and tool-result failure folding in a focused `child_agent_response_folding.rs` module while `agent_child.rs` preserves public re-exports and child-agent loop orchestration, provider turns, subagent contracts, and TUI child-agent delegation keep the same runtime shape, plus v0.1.139 isolates child-agent model routing, provider hook execution, provider call dispatch, child compaction, and prompt-too-long retry/failure handling in a focused `child_agent_provider_turn.rs` module while `agent_child.rs` preserves public re-exports and provider response folding, tool request extraction, tool result folding, subagent contracts, and TUI child-agent delegation keep the same runtime shape, plus v0.1.138 isolates child-agent loop setup, provider bootstrap, conversation seeding, approval policy seeding, and child turn-budget state in a focused `child_agent_loop_setup.rs` module while `agent_child.rs` preserves public re-exports and child-agent model routing, provider turns, tool folding, subagent contracts, and TUI delegation keep the same runtime shape, plus v0.1.137 isolates runtime approval decisions, approval handlers, and config-backed interactive approval handling in a focused `runtime_approval.rs` module while `lifecycle.rs` preserves public re-exports and approval policy resolution plus interactive approval behavior keep the same runtime shape, plus v0.1.136 isolates runtime permission request/response/handler and turn permission overlay ownership in a focused `runtime_permission.rs` module while `lifecycle.rs` preserves public re-exports and `request_permissions` execution keeps the same runtime behavior, plus v0.1.135 isolates runtime `request_user_input` handling in a focused `runtime_user_input.rs` module that owns `RuntimeUserInputRequest`, `RuntimeUserInputHandler`, JSON argument parsing, and answer/cancel `ToolResult` mapping while `lifecycle.rs` preserves public re-exports and delegates execution, plus v0.1.134 hardens runtime ORCA_HOME-scoped test infrastructure with a poison-tolerant `history::lock_test_env()` helper shared by history, server, session, thread-store, and workflow-host tests so one panicking test no longer cascades into misleading `PoisonError` failures, plus v0.1.133 focused sandbox command contexts with `WorkspaceWriteSandboxCommandContext` and `ReadOnlySandboxCommandContext` grouping workspace-write and read-only bash command construction inputs while macOS Seatbelt profile generation uses focused `WorkspaceWriteProfileContext` and `ReadOnlyProfileContext` inputs and shell sessions, command/exec, model-visible bash, network and filesystem policy flags, Unix socket allowlists, and non-interactive process preparation keep the same runtime behavior, plus v0.1.132 focused runtime bash execution contexts with `RuntimeBashSandboxContext` and `RuntimeBashOnceContext` grouping sandbox execution and one-shot shell spawning inputs while model-visible bash keeps permission-profile sandboxing, network and filesystem permission retries, cancellation, task-registry handoff, output truncation, and diagnostics, plus v0.1.131 focused readonly tool-turn boundary with `runtime_readonly_tool_turn.rs` owning readonly batch planning, hook-gated parallel execution, result recording, and grouped readonly batch/tool-turn contexts while `tool_turn.rs` keeps request cursoring, child-tool policy checks, subagent batching, readonly batch selection, and normal tool turns, plus v0.1.130 focused async subagent worker boundary with `subagent_async_worker.rs` owning async worker entrypoint execution, worker process spawning, task-registry usage writes, async result payload shaping, and worktree handoff while `subagent_execution.rs` keeps sync and batch subagent execution, plus focused protocol facade with separate command_exec, events, permissions, shell, thread, turn, and wire modules, focused ThreadStore storage facade with separate types, local JSONL, writer, projection, pagination, and live-thread modules, a server router boundary that keeps decoded `ClientOp` dispatch outside the stdio line handler while preserving existing operation handlers, focused server processor boundaries for synchronous thread query/metadata operations, turn-control operations, shell session operations, command/exec compatibility operations, permission response operations, and submit-family operations, a focused server command/exec manager module for active process records, streaming drain, stdin write, resize, termination, output-cap accounting, and permission-retry drain outcomes, a focused server active-turn manager module for active turn controls, running turn handles, finished-turn reclamation, thread-specific reclaim waits, and completed-turn metadata merge for session-scoped permission grants, a focused server permission manager module for pending runtime and command/exec permission request state plus runtime permission handler registration, a focused server shell manager module for optional runtime shell session storage, lazy task-registry-backed session manager creation, shell CRUD/read/kill/reap/wait calls, and command/exec drain compatibility borrowing, dedicated runtime-special dispatch and small-executor module for `request_permissions`, workflow IPC, subagent status, task list/stop, and workflow draft previews, managed permission-profile network domain enforcement for command/exec and model-visible bash through a loopback HTTP proxy with Codex-style `blocked-by-denylist` / `blocked-by-allowlist` diagnostics, normalized `x-proxy-host` attribution for blocked proxy requests, automatic single-shot and streaming/processId `command/exec` network permission request/retry for eligible managed proxy blocks, model-visible bash network permission request/retry for eligible active permission-profile proxy blocks through a dedicated `runtime_bash` boundary, shared filesystem sandbox-denial diagnostics and permission-request retry for eligible command/exec and model-visible bash write blocks including nested `.git/index.lock` failures, a shared runtime permission overlay merge path for file-system grants, network domain grants, and strict auto-review responses, denylist blocks that remain final diagnostics, and default local/private target blocking unless explicitly allowlisted, DNS-resolved non-public target blocking before connect, session-scoped `request_permissions` network domain grants that persist on server threads and feed command/exec proxy policy, session-scoped network denials that override permission-profile allow entries while allow overlays remain conservative, macOS command/exec Unix socket allowlists for configured permission profiles without broad network access, configurable permission-profile filesystem glob scan depth with inherited profile defaults and child-profile overrides, bounded permission-profile filesystem glob expansion for read/write/read-write command sandbox roots, dedicated runtime compaction module for prompt-budget hooks, summary persistence, and prompt-too-long recovery, inline TUI scrollback/viewport split for native terminal history plus live bottom-pane rendering, dedicated runtime tool-turn module for cursoring, batching, execution, and result folding, TUI tool approval gate owned by the runtime interaction adapter, dedicated TUI runtime interaction adapter module, dedicated TUI runtime event projection module, runtime-owned `RuntimeThread` boundary that groups `InteractiveSession` and `RuntimeSessionLifecycle` and now backs server-mode `ServerThread`, headless controller agent state, and the TUI conversation-session wrapper, site prerender server entry for crawler-visible HTML, shared tool item started/completed/status/error projection helpers, shared persisted commandExecution and fileChange history projection helpers, shared agent-message/plan/reasoning/commandExecution/fileChange/workflow lifecycle item builders for realtime server streams, tag release-gate Rust tests serialized for server-heavy contracts, stdio MCP fixture hardening for Linux release runners, active-writer JSONL polling hardening for server background-turn tests, realtime tool item error exit-code projection, MCP resource capability caching, MCP resource/template discovery with registry-level startup errors in all-server listings, MCP resource listing error aggregation, MCP resource list/read tools, server JSONL test harness hardening, structured hook action validation, tool argument schema composition validation, fuzzy model-facing file discovery, runtime-owned agent turn loop orchestration, workflow parity loop, process timeout hardening, runtime/TUI task-turn lifecycle seed, and configured permission-profile write/deny/network/special-root sandboxing, hardened inline TUI scrolling with rendered-line-info overflow detection, auto-scroll locking, CJK-aware wrap-height calculation, resume/clear scrollback resets, and post-turn mouse-scroll debounce, background memory extraction moved off the TUI render thread, and a live activity indicator bar with a refreshed status-bar layout, configurable DeepSeek reasoning effort (`high`/`max`, default `max`) through env vars, config file, and CLI arguments carried on API requests, and a stepwise TUI `/model` picker that selects model then reasoning effort with deferred apply and clean `Esc` cancellation, plus runtime-owned TUI child-agent request construction, model/cost setup, loop orchestration, provider turn handling, grouped turn-iteration, provider-cycle, provider-response, and request-scoped `RuntimeStepContext` input boundaries, tool-request extraction, tool-result folding, narrow tool-executor context, a focused RuntimeToolRouter dispatch boundary for workflow, subagent, task, permission, workflow IPC, and normal-tool routing, and a focused RuntimeNormalToolExecutor boundary for model-visible bash shell-session execution plus a grouped `RuntimeBashInvocationContext` and an injectable MCP/external/built-in fallback executor boundary, and end-to-end tool-call argument streaming progress through a `tool.call.progress` event and `ToolCallProgress` provider step with TUI progress rendering, an SSE streaming idle-timeout guard, environment-variable proxy and hook-timeout output fixes, lifecycle-owned normal-tool invocation through one runtime_normal_tool helper instead of direct executor construction, router-owned normal-tool dispatch through a grouped `RuntimeNormalToolInvocation` instead of the long roots/cancel lifecycle method, and historical projected tool completion rebuilt through shared `tool_item_projection::complete_projected_tool_item` instead of thread-store-local completed-item constructor calls, and tool approval gate inputs grouped behind `ToolApprovalGateContext` instead of a long `handle_approval` argument list, and normal tool-turn inputs grouped behind `RuntimeNormalToolTurnContext` instead of a long `run_normal_tool_turn` argument list, provider-to-tool-turn dispatch inputs grouped behind `RuntimeToolTurnsContext` instead of a long `run_tool_turns` call list, bash shell-session invocation inputs grouped behind `RuntimeBashInvocationContext` instead of a long `execute_bash_with_shell_session` call list, runtime turn-loop orchestration owned by a dedicated `runtime_turn_loop` module instead of `lifecycle.rs`, runtime turn-iteration orchestration owned by a dedicated `runtime_turn_iteration` module instead of `lifecycle.rs`, runtime turn-opening orchestration owned by a dedicated `runtime_turn_opening` module with grouped `RuntimeTurnOpeningInput` instead of `lifecycle.rs`, runtime turn-start orchestration owned by a dedicated `runtime_turn_start` module instead of `lifecycle.rs`, runtime model-route orchestration owned by a dedicated `runtime_model_route` module with grouped `RuntimeModelRouteInput` instead of `lifecycle.rs`, runtime steer application owned by a dedicated `runtime_steer` module with grouped `RuntimeSteerInput` instead of `lifecycle.rs`, runtime conversation bootstrap owned by a dedicated `runtime_conversation_bootstrap` module instead of `lifecycle.rs`, runtime turn setup owned by a dedicated `runtime_turn_setup` module instead of `lifecycle.rs`, runtime lifecycle state machine types owned by a dedicated `runtime_lifecycle` module instead of `lifecycle.rs`, and runtime tool actor context owned by a dedicated `runtime_tool_actor` module while `lifecycle.rs` preserves public re-exports
