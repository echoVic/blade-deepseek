...
4. **Permission profile persistence:** Codex preserves approval mode across
   ...
   preserved as additional readable roots, entries with `write` or
   ...
   treated as literal paths. Fully restricted Codex read allow-list enforcement,
   broader bounded glob policy support for non-deny access, and real network
   proxy/domain policy enforcement remain later expansions.
