...
Current baseline: v0.1.42 workflow parity loop, process timeout hardening, runtime/TUI task-turn lifecycle seed, and configured permission-profile write/deny/network/special-root sandboxing
...
4. **Permission profile persistence:** Codex preserves approval mode across
   ...
   default, and Codex-style domain / Unix-socket policy fields now parse into
   config but return an explicit `command/exec` error until Orca has a managed
   network proxy/enforcer. Configured `:workspace_roots` /
   ...
   `/**` filesystem entries now normalize to subtree roots, and `deny` glob
   entries such as `*.env` are expanded before command sandbox startup into
   concrete read/write deny roots, while complex `read` / `write` glob entries
   are rejected with an explicit `command/exec` error instead of being silently
   treated as literal paths. Broader bounded glob policy support for non-deny
   access and real network proxy/domain policy enforcement remain later
   expansions.
5. **Protocol item stream:** Codex SDK emits `thread.started`, `turn.started`,
   ...
   event/handler boundaries. Realtime MCP/dynamic tool item helpers now use a
   shared completed-status check before emitting success result/content items,
   and realtime file-change item helpers now share the same success-output /
   error-detail split. Non-success output is surfaced as error detail without
   also being rendered as successful content. Command-execution items
...
| MCP transport | stdio and SSE | Keep routing namespaced as `mcp__server__tool`; `startup_timeout_ms` and `tool_timeout_ms` bound startup/tool requests, and failed transports refresh for later calls |
