...
2. Add a runtime event adapter. Server-mode adapter done in v0.1.32; TUI
   ...
   - Workflow terminal notifications now flow through runtime
     ...
     adapting back to `TuiEvent::WorkflowTasksUpdated`.
...
| P1 | TUI event adapter | Assistant deltas, usage, model routing notices, errors, session completion, tool requested/completed, plan updated, subagent started/completed, approval prompts/resolution notices, request-user-input prompts/results, verification started/completed notices, workflow terminal notifications, and workflow task-list/progress refreshes now flow through runtime `EventFactory`/handler boundaries; next verify no renderer-owned orchestration remains on the prioritized path | Medium |
