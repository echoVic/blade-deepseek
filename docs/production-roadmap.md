...
| P1 | TUI event adapter | Assistant deltas, usage, errors, session completion, tool requested/completed, plan updated, subagent started/completed, approval prompts/decisions, request-user-input prompts/results, workflow terminal notifications, and workflow task-list/progress refreshes now flow through runtime `EventFactory`/handler boundaries; next verify no renderer-owned orchestration remains on the prioritized path | Medium |
