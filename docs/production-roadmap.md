...
| P1 | Storage-neutral ThreadStore | Codex keeps thread persistence behind a dedicated `thread-store` crate; Orca now exposes a `thread_store` module for `ThreadStore`, `JsonlThreadStore`, live threads, message/turn/item projections, pagination, and filters, with `SessionStore` retained as a compatibility alias; live server message/turn/item projection and pagination now go through the boundary while JSONL internals remain in `history`; next move implementation-owned JSONL helpers behind the module boundary without changing server/session behavior | Medium |
