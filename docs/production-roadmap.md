...
5. **Protocol item stream:** Codex SDK emits `thread.started`, `turn.started`,
   ...
   share MCP tool name parsing, JSON argument parsing, MCP result shaping, and
   camelCase tool error object helpers. Realtime MCP/dynamic tool item helpers
   also use a shared completed-status check before emitting success
