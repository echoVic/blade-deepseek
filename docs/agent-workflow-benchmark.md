...
| **Observability: token/elapsed/agent count** | ✅ PRESENT | `/workflows` now receives live agent/phase progress counts, renders elapsed async subagent rows, and shows selected workflow phase fallback/error detail plus workflow-agent retry/failure detail, per-agent lifecycle timestamps/elapsed time, token usage, and cost | ✓ |
