...
| **Agent Budget/Token Tracking** | ✅ PRESENT | `CostTracker` totals are persisted per workflow child agent, shown in `/workflows`, and `[workflows] max_agent_tokens = ...` fails child agents whose reported usage exceeds the hard token budget. A run may also set `tokenBudget`; settled child usage (including retries) is accumulated into `{ total, spent, remaining }`, new child agents are rejected after exhaustion, and the completion notification/status line reports the snapshot. | ✓ |
...
| Token usage tracking | ✅ Yes | `CostTracker` per child agent; workflow child totals are persisted and shown in `/workflows`; `[workflows] max_agent_tokens` enforces per-agent hard token budgets; `tokenBudget` adds a run-level hard gate with `total`, `spent`, and `remaining` output |
...
9. **Run-level token budget**: ✅ Implemented. Workflow input accepts `tokenBudget` (also on draft runs); settled child usage, including retries and resumed usage, is exposed as `{ total, spent, remaining }`. New child agents are rejected after the hard limit, and the final status line marks the 80% warning threshold. Files: `workflow_types.rs`, `workflow/runner.rs`, `workflow_execution.rs`, `registry.rs`.
...
13. **Resume/fork stress coverage**: ✅ Implemented. Complex resume coverage now exercises failed phases, fallback recovery agents, cached downstream agents, cached per-agent rows, progress totals that include cache hits, and chained resume from cached rows. Files: `workflow/state.rs`, `workflow/runner.rs`, `tests/workflow_runtime_contract.rs`.
