...
| **Agent Teams** | ⚠️ PARTIAL+ | Workflow agents can carry/display a team label and `[workflows.teams.<name>]` can override `max_agent_tokens` and `max_agent_retries`; role-scoped tool access remains open | P2 |
...
| **Shared Task List** | ⚠️ PARTIAL | Workflow scripts can use `createTaskList(name, items)`, `claimTask(name, opts)`, `completeTask(name, id, result, opts)`, and `listTasks(name)` to coordinate parallel worker prompts through an orchestrator-owned task list; direct child-agent task claiming remains open | P2 |
...
| **Structured Agent Output** | ⚠️ PARTIAL+ | Workflow agents return JSON `Value` and support `agent(prompt, { schema })` validation for `type`, `required`, and `properties`; subagent tool output remains text-only without schema validation | P2 |

**Overall gap**: Orca's **workflow system** is architecturally capable of concurrent agent fan-out (confirmed 8+ agents) with observability (phase tracking, agent statuses, team labels, retry attempts, lifecycle timestamps, per-agent token usage, per-agent hard token budgets, per-team token/retry budgets, and bounded child-agent retry). The TUI now has `/workflows` for task-oriented progress and `/agents` for a dedicated workflow-agent dashboard across runs, while model-facing subagents have worker-backed async/status handles in headless/`exec` and optional worktree isolation for file-writing tasks. Workflow agents can also opt into worktree isolation, phases can opt into continue-on-failure, explicit fallback-value recovery, or async recovery functions, workflow scripts can pass messages and task-list state between agent calls through orchestrator-owned channels, and workflow agent outputs can be schema-validated. The remaining gaps are role-scoped team tool access, direct child-agent mailbox/tool IPC, durable child-agent task claiming, model-facing subagent typed output, and resume/fork stress coverage.
...
| P2: Agent communication | ⚠️ Partially implemented | `workflow/host.mjs`, `tests/workflow_host_contract.rs` | Remaining: direct child-agent mailbox/tool IPC and durable cross-run channels |
| P2: Shared task list | ⚠️ Partially implemented | `workflow/host.mjs`, `tests/workflow_host_contract.rs` | Remaining: direct child-agent task claiming and durable cross-run queues |
...
| Shared task list | ⚠️ Partial | Workflow-level task lists let orchestration scripts create, claim, complete, and inspect shared work items for parallel worker prompts; direct child-agent queue tools remain open |
...
| Structured typed output | ⚠️ Partial+ | Workflow `agent(..., { schema })` validates returned JSON against a JSON Schema subset (`type`, `required`, `properties`); model-facing subagent output remains text-only |
...
9. **Shared task list**: ⚠️ Partially implemented. Workflow scripts can use `createTaskList(name, items)`, `claimTask(name, opts)`, `completeTask(name, id, result, opts)`, and `listTasks(name)` to coordinate parallel worker prompts through an orchestrator-owned work queue. Remaining work: direct child-agent task-claiming tools and durable cross-run queues. Files: `workflow/host.mjs`, `tests/workflow_host_contract.rs`.

10. **Structured typed output**: ⚠️ Partially implemented. Workflow agents can pass `schema` in `agent(prompt, { schema })`, and the runner validates the returned JSON wrapper against `type`, `required`, and `properties`, failing the agent/run with a clear schema error when output mismatches. Remaining work: model-facing subagent typed output and broader schema features if needed. Files: `workflow/runner.rs`, `tests/workflow_runtime_contract.rs`.

11. **Agent teams**: ⚠️ Partially implemented. Workflow agents can use `agent(prompt, { team })`, dashboards preserve that label, and `[workflows.teams.<name>]` can override `max_agent_tokens` and `max_agent_retries` for team-level execution policy. Remaining work: role-scoped tool access / permission policy. Files: `config/mod.rs`, `config/file.rs`, `workflow/runner.rs`, `tests/workflow_runtime_contract.rs`.

12. **Resume/fork stress coverage**: Exercise complex workflow resume/fork paths across phases, cached agents, failed phases, and fallback recovery. Files: `workflow/state.rs`, `workflow/runner.rs`, `tests/workflow_runtime_contract.rs`.
