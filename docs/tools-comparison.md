...
| 维度 | Claude Code | Codex CLI | Orca v0.1.25 |
|------|-------------|-----------|--------------|
...
- `get_goal`、`create_goal` 和 `update_goal` 只应在 TUI goal turn 中给模型使用。
...
- `crates/orca-tools/src/update_goal.rs` — 持久化 goal 读取、创建和状态更新工具。
...
Orca v0.1.25 的工具系统已经完成从“硬编码工具枚举”到“规格驱动工具注册表”的关键迁移，并补上了 Markdown skills、TUI 结构化用户输入以及更接近 Codex 的 goal 工具分层。发布链路也已补上 GitHub Release、npm registry 与 `npm exec` 外部校验的重试等待，避免 npm 发布后的短暂传播窗口误判失败。接下来最值得投入的是 shell session/PTY、worktree 隔离、多格式读取和更完整的 app/server SDK 面。
