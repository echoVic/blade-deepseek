...
v0.1.11 基线后，Orca 的工具系统已经从早期的硬编码分发，升级为“规格驱动”的注册表模型。每个工具都有统一的 `ToolSpec`，包含名称、别名、JSON Schema、能力集合、展示方式、可见性和并发安全信息。运行时审批、TUI 展示、provider tool schema、MCP 工具和外部自定义工具都围绕同一套规格工作。

这次变化的核心目标是接近 Codex CLI 的工具系统思路：工具不是散落在 prompt、审批和执行器里的字符串列表，而是一个可以解析、过滤、授权、渲染和扩展的统一能力表。
...
| 维度 | Claude Code | Codex CLI | Orca v0.1.11 |
|------|-------------|-----------|--------------|
...
| 结构化 ask-user | 只有审批确认，没有通用问题工具 | P1 |
...
Orca v0.1.11 的工具系统已经完成从“硬编码工具枚举”到“规格驱动工具注册表”的关键迁移。接下来最值得投入的是 P1 上下文可靠性、P2 日常交互体验和 skills 体系；shell session、worktree 隔离和多格式读取仍然重要，但应排在这些基础补丁之后。
