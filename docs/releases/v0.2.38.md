...
- **Runtime lifecycle CI**: normal shell-execution contract tests now create a
  temporary trusted workspace, preserving Linux fail-closed coverage for
  unknown folders without weakening the sandbox rule.
...
```bash
node scripts/release/verify-published.mjs --version 0.2.38 \
...
```bash
npm install -g @blade-ai/orca@0.2.38
```
