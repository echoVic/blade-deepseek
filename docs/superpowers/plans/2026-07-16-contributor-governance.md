...
```sh
ruby -e 'require "yaml"; types = %w[markdown input textarea dropdown checkboxes]; fields = %w[input textarea dropdown checkboxes]; Dir[".github/ISSUE_TEMPLATE/*.{yml,yaml}"].sort.each { |path| doc = YAML.safe_load(File.read(path), [], [], false); abort("#{path}: expected mapping") unless doc.is_a?(Hash); if File.basename(path) == "config.yml"; abort("#{path}: invalid blank_issues_enabled") unless [true, false].include?(doc["blank_issues_enabled"]); links = doc["contact_links"]; abort("#{path}: invalid contact_links") unless links.is_a?(Array) && links.all? { |link| link.is_a?(Hash) && %w[name url about].all? { |key| link[key].is_a?(String) } }; puts "ok #{path}"; next; end; abort("#{path}: invalid name") unless doc["name"].is_a?(String); abort("#{path}: invalid description") unless doc["description"].is_a?(String); body = doc["body"]; abort("#{path}: body must be an array") unless body.is_a?(Array); ids = body.map { |item| item.is_a?(Hash) ? item["id"] : nil }.compact; abort("#{path}: invalid or duplicate ids") unless ids == ids.uniq && ids.all? { |id| id.is_a?(String) && id.match?(/\A[a-z0-9_-]+\z/) }; body.each { |item| abort("#{path}: body item must be a mapping") unless item.is_a?(Hash); type = item["type"]; abort("#{path}: unsupported type #{type.inspect}") unless types.include?(type); if fields.include?(type); attributes = item["attributes"]; abort("#{path}: invalid attributes for #{type}") unless attributes.is_a?(Hash) && attributes["label"].is_a?(String); end; if %w[dropdown checkboxes].include?(type); options = item["attributes"]["options"]; abort("#{path}: invalid options for #{type}") unless options.is_a?(Array) && !options.empty?; end; abort("#{path}: checkbox component validations are unsupported") if type == "checkboxes" && item.key?("validations"); validations = item["validations"]; if validations; abort("#{path}: invalid validations") unless %w[input textarea dropdown].include?(type) && validations.is_a?(Hash) && (validations.keys - ["required"]).empty? && [true, false].include?(validations["required"]); end; if type == "checkboxes"; abort("#{path}: invalid checkbox options") unless item["attributes"]["options"].all? { |option| option.is_a?(Hash) && option["label"].is_a?(String) && (option.keys - %w[label required]).empty? && (!option.key?("required") || [true, false].include?(option["required"])) }; end }; puts "ok #{path}" }'
```

Expected: one `ok` line for each of the four Issue template files and exit code `0`.
...
Expected: four `ok` lines, then no whitespace errors.
