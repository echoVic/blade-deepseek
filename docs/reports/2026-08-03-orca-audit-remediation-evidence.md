...
|---|---|---|---|---|---|
| C1 | Goal actor synchronous replies can block the single-thread host | `GoalRuntimeHandle::request` calls unbounded `reply_rx.recv()` in `crates/orca-runtime/src/goal_actor.rs`; callers are inside the async `ThreadActor` loop | `goal_actor_request_times_out_with_typed_error`; `goal_store_does_not_block_thread_actor` | Not yet run | Not yet run |
