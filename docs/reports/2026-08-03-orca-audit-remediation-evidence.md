...
|---|---|---|---|---|---|
| C1 | Goal actor synchronous replies can block the single-thread host | `GoalRuntimeHandle::request` called unbounded `reply_rx.recv()` in `crates/orca-runtime/src/goal_actor.rs`; callers are inside the async `ThreadActor` loop | `goal_actor_request_times_out_with_typed_error`; `goal_store_does_not_block_thread_actor` | `3c746a408` bounds replies with a typed timeout; async caller migration remains | Timeout regression passed; all 15 goal-actor tests passed; `cargo check -p orca-runtime --locked` passed. Tokio-actor responsiveness proof not yet run. |
| C2 | Host supervisor performs session store and unbounded history IO on its async loop | Direct store calls in `crates/orca-runtime/src/runtime_host.rs`; `list_threads` materializes the complete store | `supervisor_remains_responsive_while_store_is_blocked`; `list_threads_applies_default_bound` | Not yet run | Not yet run |
